...
class Parser:
    ...
    def atom(self):
        ...
        elif tok.matches(TT_KEYWORD, 'NIF'):
            nif_expr = res.register(self.nif_expr())
            if res.error: return res
            return res.success(nif_expr)
    ...
    def nif_expr(self):
        ...
        if not self.current_tok.matches(TT_KEYWORD, 'NIF'):
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                f"Expected 'NIF'"
            ))
        ...
        return res.success(NifNode(cases, else_case))
...
class List(Value):
    ...
    def get_comparison_lt(self, other):
        if isinstance(other, List):
            p1 = randrange(0, len(other.elements))
            p2 = randrange(0, len(other.elements))
            if len(self.elements) < len(other.elements):
                print("Warning, first list should be bigger than second!")
            crossed = self.copy()
            if p1 == p2:
                return crossed, None
            elif p1 > p2:
            ...
            for i in range(len(self.elements)):
                ...
                if i > p1 and i < p2:
                    crossed.elements[i] = other.elements[i]
                    # crossed.elements.append(other.elements[i])
                # else:
                #     crossed.elements.append(self.elements[i])
...
class BuiltInFunction(BaseFunction):
    ...
    def execute_readfromfile(self, exec_ctx):
        Text = []
        f = open(str(exec_ctx.symbol_table.get('filename')), "r")
        Text.append(f.readlines())
        f.close()
        print("Reading from file...")
        return RTResult().success(List(Text))

    execute_readfromfile.arg_names = ['filename']
...
BuiltInFunction.readfromfile = BuiltInFunction("readfromfile")
...
class Interpreter:
    ...
    def visit_NifNode(self, node, context):
        res = RTResult()
...
global_symbol_table.set("READFROMFILE", BuiltInFunction.readfromfile)
